...
@pytest.mark.parametrize(
    ...
    [
        ...
        (rel_set, ("det", 5), {}, [Msg("read", "det"), Msg("set", "det", 5, group=None)]),
        (rel_set, ("det", 5), {"group": "A"}, [Msg("read", "det"), Msg("set", "det", 5, group="A")]),
        (
            rel_set,
            ("det", 5),
            {"group": "A", "wait": True},
        ...
        (kickoff, ("foo",), {"custom": 5}, [Msg("kickoff", "foo", group=None, custom=5)]),
        (collect, ("foo",), {}, [Msg("collect", "foo", stream=False, return_payload=True, name=None)]),
        ...
        (subscribe, ("all", "func_placeholder"), {}, [Msg("subscribe", None, "func_placeholder", "all")]),
...
def test_stub_plans(plan, plan_args, plan_kwargs, msgs, hw):
    # de-reference
    plan_args = tuple(getattr(hw, v, v) if isinstance(v, str) else v for v in plan_args)
    plan_kwargs = {k: getattr(hw, v, v) if isinstance(v, str) else v for k, v in plan_kwargs.items()}
    msgs = [
        Msg(m.command, getattr(hw, m.obj, m.obj) if isinstance(m.obj, str) else m.obj, *m.args, **m.kwargs)
        for m in msgs
    ]
...
def test_descriptor_layout_from_monitor(RE, hw):
    ...
    RE([Msg("open_run"), Msg("monitor", det, name=det.name), Msg("unmonitor", det), Msg("close_run")], collect)

    (descriptor,) = collector
    assert descriptor["object_keys"] == {det.name: list(det.describe().keys())}
    assert descriptor["data_keys"] == {k: {**v, "object_name": det.name} for k, v in det.describe().items()}
    ...
    timestamps = {key: val["timestamp"] for key, val in det.read_configuration().items()}
...
def test_suspend():
    ...
    expected = [Msg("install_suspender", None, 1), Msg("null"), Msg("remove_suspender", None, 1)]
...
def test_per_step(RE, hw):
    ...
    RE(scan([hw.det], hw.motor, -1, 1, hw.motor2, -1, 1, 3, per_step=one_nd_step))
    RE(inner_product_scan([hw.det], 3, hw.motor, -1, 1, per_step=one_nd_step))
    RE(inner_product_scan([hw.det], 3, hw.motor, -1, 1, hw.motor2, -1, 1, per_step=one_nd_step))
    ...
    with pytest.raises(TypeError) as excinfo:
        RE(scan([hw.det], hw.motor, -1, 1, hw.motor2, -1, 1, 3, per_step=one_1d_step))
...
def test_custom_stream_name(RE, hw):
    ...
    def new_per_step(detectors, motor, step):
        return (yield from one_1d_step(detectors, motor, step, take_reading=new_trigger_and_read))
